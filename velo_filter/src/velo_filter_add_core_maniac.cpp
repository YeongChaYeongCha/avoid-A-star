...
VelodyneFilterAdd::VelodyneFilterAdd(ros::NodeHandle &nh){
	...
	pub_vox_sor_cloud_=nh.advertise<sensor_msgs::PointCloud2>("/vox_sor_points_no_ground", 1000);
...
void VelodyneFilterAdd::cloud_cropbox(const pcl::PointCloud<VPoint>::ConstPtr in, const pcl::PointCloud<VPoint>::Ptr out){
	...
	region.setMax(Eigen::Vector4f(6, 3.5, 2.5, 1));
	region.setInputCloud(in);
	region.filter(*out);
	
	std::vector<int> indices;
	pcl::CropBox<VPoint> roof(true);
	roof.setMin(Eigen::Vector4f(-1.0, -0.60, -1.2, 1));
...
void VelodyneFilterAdd::cloud_cb(const sensor_msgs::PointCloud2ConstPtr &in_cloud_ptr){
	...
	sensor_msgs::PointCloud2 vox_sor_msg;
	pcl::toROSMsg(*sor_filtered_pointcloud, vox_sor_msg);
	vox_sor_msg.header.stamp=in_cloud_ptr->header.stamp;
	vox_sor_msg.header.frame_id=in_cloud_ptr->header.frame_id;
	pub_vox_sor_cloud_.publish(vox_sor_msg);
